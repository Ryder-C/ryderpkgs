...
in
  stdenv.mkDerivation {
    ...
    installPhase = ''
      ...
      # Compute store app dir (where upstream sources live)
      SCRIPT_PATH="$(${coreutils}/bin/readlink -f "$0")"
      OUT_DIR="$(dirname "$(dirname "$SCRIPT_PATH")")"
      STORE_APP_DIR="$OUT_DIR/share/steam-presence"
